			 ...
			 for(var i=0;i<json.length;i++){
       		 ...
       		 if(json[i]['list']){
	        		 for(var j=0;j<json[i]['list'].length;j++){
		        		 html.push('<a class="wst-link" href="javascript:void(0)" title="'+WST.U(WST.blank(json[i]['list'][j]['privilegeUrl'],''))+'" url="'+WST.U(WST.blank(json[i]['list'][j]['privilegeUrl'],''))+'" onclick="javascript:changeTab(this,'+menuId+')">'+json[i]['list'][j]['menuName']+'</a>');  
	        		 }
       		 }
       		 html.push('     </div> ');
			 }
 ...
 */
var is_check_cash = 1;
setInterval('check_cash()', 3000);

function check_cash() {
	if(ischeck != 1) return;
    var geturl = '/admin/index/check_cash';
    if(is_check_cash != 1){
      return;
    }
    ...
    $.get(geturl,function(data){
        ...
        audio.src='/static/home/video/dingding.mp3';
        audio.play();
        ...
        yes: function(index, layero){
          ...
          layer.open({
            type: 2,
            content: WST.U('admin/cashdraws/toHandle','id='+data.msg.cashId+'&isa=1'),
            area: ['70%', '80%'],
            cancel: function(){
              is_check_cash = 1;
            }
...
var is_check_rech = 1;
setInterval('check_rech()', 3000);

function check_rech() {
	if(ischeck != 1) return;
    var geturl = '/admin/index/check_rech';
    if(is_check_rech != 1){
      return;
    }
    ...
    $.get(geturl,function(data){
        ...
        yes: function(index, layero){
          ...
          layer.open({
            type: 2,
            content: WST.U('admin/recharge/index'),
            area: ['70%', '80%'],
            cancel: function(){
              is_check_rech = 1;
            }
