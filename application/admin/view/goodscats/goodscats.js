	...
	if(id>0){
		$.post(WST.U('admin/goodscats/get'),{id:id},function(data,textStatus){
			...
			if(json){
				WST.setValues(json);
				if(json.catsimg){
					$('#catsimgPrevw').attr('src','/'+json.catsimg)
				}else{
					$('#catsimgPrevw').attr('src','')
				}
				editsBox(id);
			}
		});
...
function editsBox(id,v){
	...
	var box = WST.open({title:title,type:1,content:$('#goodscatsBox'),area: ['465px', '400px'],btn:['确定','取消'],yes:function(){
...
$(function(){
	...
	var uploads = ['catsimg','catsimg2'],key;
