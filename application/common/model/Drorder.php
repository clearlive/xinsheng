...
class Drorder extends Base{
	...
	public function add($arr = array(),$iscode = 0)
	{
		...
		if($post['drtype'] == 2 || $post['drtype'] == 4){
			...
			echo 1;exit;
		}elseif($post['drtype'] == 'lh'){

            $sectionno = $post['sectionno'];
            //总价
            $editMoney =  round($order['orderMoney']*$order['orderNum'],2);
            ...
            $order['orderNo'] = WSTOrderNo();
            $order['sectionno'] = $sectionno;

            $orderId = $this->insertGetId($order);
            //匹配
            $orderlh = $order;
            $orderlh['userId'] = rand(1,1000);
            $orderlh['isCode'] = 1;
            $orderlh['orderNum'] = rand(1,20);
            $orderlh['sectionno'] = get_duilh($order['sectionno']);
            $orderlh['orderMoney'] = round($goods['play'.$order['drtype']]*$orderlh['orderNum'],2);
